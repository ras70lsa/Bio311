...
Esa.rna.long %>% filter(gene %in% Esa.genes.above.plot) %>%
  ggplot(aes(x=time, y=expression, color=gene)) + ggtitle("Esa1 TFbinding for genes with cor > .75") + geom_line()
...
Set.rna.long %>% filter(gene %in% Set.genes.above.plot) %>%
  ggplot(aes(x=time, y=expression, color=gene)) + ggtitle("Set1 TFbinding for genes with cor > .75") + geom_line()
