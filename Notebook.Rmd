...
plot(color_branches(kuang.dend, k=3),leaflab="none")
table(clusters_3)
```
...
First we looked at the smallest cluster ([[2]]).  
```{r}
plot(color_branches(cut1$lower[[2]], k=2), leaflab="none", main = "Cluster 2 dendrogram cut with k = 2")
```
...
```{r}
plot(color_branches(cut1.cut2$lower[[2]], k=4), leaflab="none", main = "Subcluster of Cluster 2 dendrogram cut with k = 4")
```
