...
clusterf1.genes.first10 <- clusterf1.genes[1:10]
```

```{r}
kuang_t.time.long.fitered %>%
  filter(gene %in% clusterf1.genes.first10) %>%
...
clusterf2.genes.first10 <- clusterf2.genes[1:10]
kuang_t.time.long.fitered %>%
  filter(gene %in% clusterf2.genes.first10) %>%
  ggplot(aes(x = time, y = expression, color = gene)) + geom_line()
```
...
clusterf3.genes.first10 <- clusterf3.genes[1:10]
kuang_t.time.long.fitered %>%
  filter(gene %in% clusterf3.genes.first10) %>%
  ggplot(aes(x = time, y = expression, color = gene)) + geom_line()
```
...
clusterf2.1.genes.first10 <- clusterf2.1.genes[1:10]

cluster2.1 <- kuang_t.time.long.fitered %>%
  filter(gene %in% clusterf2.1.genes.first10) %>%
  mutate(cluster = 2)
