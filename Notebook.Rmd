...
ggplot(kuang_t.var, aes(var)) + geom_histogram()
...
Next, we grabbed the gene names from the first final cluster.

```{r}
clusterf1.genes <- as.character(labels(cluster1.cut1$lower[[1]]))
clusterf1.genes.first4 <- clusterf1.genes[1:20]
```

```{r}
kuang_t.time.long.fitered %>%
  filter(gene %in% clusterf1.genes.first4) %>%
  ggplot(aes(x = time, y = expression, color = gene)) + geom_line()
```

From this plot it is clear the first couple of genes in this cluster have similar expression over time.

We repeated this step from our other clusters with similar results.

```{r}
clusterf2.genes <- as.character(labels(cluster1.cut1$lower[[2]]))
clusterf2.genes.first4 <- clusterf2.genes[1:4]
kuang_t.time.long.fitered %>%
  filter(gene %in% clusterf2.genes.first4) %>%
  ggplot(aes(x = time, y = expression, color = gene)) + geom_line()
```
...
clusterf3.genes.first4 <- clusterf3.genes[1:4]
kuang_t.time.long.fitered %>%
  filter(gene %in% clusterf3.genes.first4) %>%
  ggplot(aes(x = time, y = expression, color = gene)) + geom_line()
```
...
clusterf2.1.genes.first4 <- clusterf2.1.genes[1:20]

cluster2.1 <- kuang_t.time.long.fitered %>%
  filter(gene %in% clusterf2.1.genes.first4) %>%
  mutate(cluster = 2)
...
```
Two cluster were apparent.  We then cut the tree to expore them more.
```{r}
...
Next we added color.
```{r}
plot(color_branches(rnaseq.clusters.cluster1$lower[[1]], k=4),leaflab="none", main = "RNAseq Cluster 1 Subcluster 1 with color")
```
Next was a cut to extract these four clusters.
```{r}
rnaseq.clusters.cluster1.clusterAgain <- cut(rnaseq.clusters.cluster1$lower[[1]], h = 1)
...
```{r}
plot(color_branches(rnaseq.clusters$lower[[2]], k=5),leaflab="none", main = "RNAseq Cluster 2 with color")

```
None of these clusters needed to be explored further.  We did extract them however.
```{r}
rnaseq.clusters.cluster2 <- cut(rnaseq.clusters$lower[[2]], h = 1.5)
...
kuang_t.long %>%
  ...
  ggplot(aes(x = time, y = gene)) + 
    ...
    labs(title = "K-medoids (k=1) Clustering of Kuang et al. Data\nCluster 1") + 
...
kuang_t.long %>%
  ...
  ggplot(aes(x = time, y = gene)) + 
    geom_tile(aes(fill = expression)) +
    scale_fill_gradientn(colors=color.scheme, limits=c(-2,2)) + 
    labs(title = "K-medoids (k=2) Clustering of Kuang et al. Data\nCluster 2") + 
    theme(axis.text.y = element_text(size = 6))  # set size of y axis labels
```
### Heatmap - cluster 3
```{r}
...
### Heatmap - cluster 1
```{r}
rnaseq.unique.genes <- colnames(rnaseq_t_zeros) %>% unique()
rnaseq_t.unique = subset(rnaseq_t_zeros.time, select=c(rnaseq.unique.genes, "time"))
